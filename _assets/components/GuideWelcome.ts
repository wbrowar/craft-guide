...
export class GuideWelcome extends LitElement {
   ...
   */
  @state()
  private _animationXDirection = 1
  ...
  render() {
    return html`
      <div class="guide-welcome-scene-wrapper">
        ...
        ${this._playing
          ? html`<button type="button" @click="${this._pause}">Pause</button>`
          : html`<button type="button" @click="${this._play}">Play</button>`}
      </div>
